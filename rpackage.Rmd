...
predict.blbglm <- function(object, newdata, confidence = FALSE, level = 0.95, ...) {
  ...
  new_as_col <- t(X) #convert row to column
  
  # Function to multiply new data by coefficient to predict using logistic model equation
  func <- function(x) {
      x*new_as_col
    }
...
pred_sub_ci <- pred_sub_list %>% 
  map(function(x){apply(x, 1, quantile, c(0.025, 0.975))}) #%>%
  #map(function(x){apply()})
...
```
```{r}
pred_sub_ci <- data.frame(pred_sub_list) %>% map(function(x){apply(x, 2, quantile,c(0.025, 0.975))}) 

pred_as_df <- data.frame(pred_sub_list)
print(pred_as_df)
apply(pred_as_df, 2, quantile,c(0.025, 0.975))

print(pred_sub_list)
pred_sub_list %>%
  map(function(x){data.frame(Reduce(cbind, x))})

map(pred_sub_list, Reduce(cbind, pred_sub_list))

pred_sub_ci <- pred_as_df %>% map(function(x){apply(x, 2, quantile,c(0.025, 0.975))})


quantile(pred_as_df, 2, c(0.25, 0.75))

print(pred_sub_list)
pred_sub_ci <- data.frame(pred_sub_list) %>% map(function(x){quantile(x, c(0.025, 0.975))}) 
print(pred_sub_ci)
