...
pred_sub_list <- coef_sub %>%     
  ...
  #rowMeans(as.data.frame(lapply(temp, colSums)))
  #map(colSums)  

pred_sub_mean = pred_sub_list %>% map(function(x){rowMeans(as.data.frame(lapply(x, colSums)))})
#pred_sub_mean <- pred_sub %>% map(mean) 

pred <- Reduce("+", pred_sub_mean)/length(pred_sub_mean)
pred
#temp = pred_sub %>% map(combine)
#pred_sub
```
```{r}
pred_sub_ci <- pred_sub %>% map(function(x){apply(x, 2, quantile,c(0.025, 0.975))}) 
pred_ci <- Reduce("+", pred_sub_ci) / length(pred_sub_ci)
pred_ci
```
