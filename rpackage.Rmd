...
sigma.blbglm <- function(object, confidence = FALSE, level = 0.95, ...) {
  ...
  sigma_list <- est %>% map(function(x){map(x,function(a){a[[2]]})})
  sigma_mat <- sigma_list %>% map(function(x){as.matrix(matrix(unlist(x), nrow=length(unlist(x[[1]]))))})
  
  mean_list = sigma_mat %>% map(mean)
  sigma_mean = Reduce("+", mean_list) / length(mean_list)

  ci_list = sigma_mat %>% map(function(x){apply(x, 1, quantile, c(0.025, 0.975))})
...
confint.blbglm <- function(object, parm = NULL, level = 0.95, ...) {
  if (is.null(parm)) {
   ...
   t(Reduce("+", ci_list) / length(ci_list))
}
...
coef(bank_model)
confint(bank_model)
confint(bank_model, c("pdays", "age"))
sigma(bank_model)
